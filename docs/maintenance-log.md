...
### 2026-07-06 Android 周免未进入 GitHub Actions 领取队列

- 现象：
  - GitHub Actions 能领取官方周免接口返回的 PC 游戏，但 Android 周免商品如 `Sophia the Traveler Android` 没有进入自动领取队列。
  - 直接打开 Android 商品页时，既有领取状态机已能处理 `Device not supported`、`ADD TO LIBRARY` 和已入库状态，问题不在结账流程本身。
- 根因判断：
  - 当前发现源依赖 Epic 官方 `freeGamesPromotions` 周免接口；该接口只返回 PC 周免商品，不稳定覆盖 Android 或特殊限免商品。
  - Android 商品 URL 没有被配置进队列时，后续浏览器领取逻辑没有机会执行。
- 改动文件：
  - `app/settings.py`
  - `app/services/epic_games_service.py`
  - `.github/workflows/epic-gamer.yml`
  ...
  - `docs/maintenance-log.md`
- 处理结果：
  - 新增 `EXTRA_PROMOTION_URLS` 配置，支持用英文逗号、分号或换行配置额外 Epic 商品链接。
  - `get_promotions()` 会在官方接口结果后追加这些额外 URL，并去重同一商品链接，避免影响已有 PC 周免发现逻辑。
  - GitHub Actions 已透传同名 Secret，可在 fork 仓库 Secrets 中配置 Android 周免链接后随手动或定时任务一起领取。
